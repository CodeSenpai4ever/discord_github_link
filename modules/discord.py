...
class discord():
    def get_oauth_url():
        ...
        params = {
                ...
                'response_type': 'code',
                'state': state,
                'scope': 'role_connections.write identify connections',
                'prompt': 'consent',
        }
    ...
    # Given a user based access token, fetch connections for the current user.
    def get_connections_data(tokens):
        ...
        if response.ok:
            ...
            raise Exception(f'Error fetching user connections: [{response.status_code}] {response.text}')
